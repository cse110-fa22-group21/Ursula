...
function openEditForm(id) {
	...
	for (var i = 0; i < taskList.length; i++) {
		...
		if (taskList[i].id == id) {
			document.getElementById("editTask").innerText =
				"Editing " + taskList[i].name;
			document.getElementById("taskNameFieldEdit").value =
				taskList[i].name;
			...
			document.getElementById("typeTaskFieldEdit").value =
				taskList[i].type;
	...
	document
		.getElementsByClassName("deleteEditButton")[0]
	...
	document
		.getElementsByClassName("submitEditButton")[0]
		.addEventListener("click", () => {
			saveData(id);
		});
